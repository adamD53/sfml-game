...
{
    for (auto* layerNode = map->FirstChildElement("layer"); layerNode;
         layerNode = layerNode->NextSiblingElement("layer"))
    {
        ...
        for (const auto gid : gids)
            if (gid && gid < m_GidToTileset.size())
            {
        ...
        for (uint32_t y = 0; y < m_MapHeight; ++y)
        {
            for (uint32_t x = 0; x < m_MapWidth; ++x)
            {
                ...
                v[0] = sf::Vertex{{px, py}};
                v[0].texCoords = {tx, ty};
                v[1] = sf::Vertex{{px + ts.tileW, py}};
                v[1].texCoords = {tx + ts.tileW, ty};
                v[2] = sf::Vertex{{px + ts.tileW, py + ts.tileH}};
                v[2].texCoords = {tx + ts.tileW, ty + ts.tileH};
                v[3] = sf::Vertex{{px, py}};
                v[3].texCoords = {tx, ty};
                v[4] = sf::Vertex{{px + ts.tileW, py + ts.tileH}};
                v[4].texCoords = {tx + ts.tileW, ty + ts.tileH};
                v[5] = sf::Vertex{{px, py + ts.tileH}};
                v[5].texCoords = {tx, ty + ts.tileH};
    ...
    std::unordered_map<std::string, std::unique_ptr<sf::Texture>>& textures) -> void
{
    ...
    for (auto* ts = map->FirstChildElement("tileset"); ts; ts = ts->NextSiblingElement("tileset"))
    {
        ...
        if (!tileName)
        {
            std::println(stderr, "Error reading tileset name.");
            break;
        }
...
auto Terrain::Load(const std::string& tmx_file, std::unordered_map<std::string, std::unique_ptr<sf::Texture>>& textures)
    -> void
{
